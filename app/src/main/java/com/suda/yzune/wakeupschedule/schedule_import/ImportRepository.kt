...
class ImportRepository(url: String) {
    var VIEWSTATE_POST_CODE = MutableLiveData<String>()
    ...
    var login_response = MutableLiveData<String>()
    ...
    fun login(xh: String, pwd: String, code: String) {
        importService.login(
            ...
            override fun onFailure(call: Call<ResponseBody>?, t: Throwable?) {
                login_response.value = "Failure"
            }

            override fun onResponse(call: Call<ResponseBody>?, response: Response<ResponseBody>?) {
                login_response.value = response?.body()!!.string()
                Log.d("返回", login_response.value)
                VIEWSTATE_POST_CODE.value = parseViewStateCode(login_response.value!!)
            }
        })
    }
    ...
    fun toSchedule(xh: String, name: String, year: String, term: String) {
        importService.getSchedule(
                ...
                event_target = "xnd",
                event_argument = "",
                //view_state = VIEWSTATE_POST_CODE.value!!,
